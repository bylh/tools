...
function diffHtml(left, right, config = {}, callback) {
    ...
    // 根据左右html创建dom树
    let leftFrag = document.createRange().createContextualFragment(left);
    let rightFrag = document.createRange().createContextualFragment(right);
    // map结构保存左右dom树的遍历节点，方便后续处理
    let leftNodeMap = new Map(); // 保存左侧必要的节点
    let rightNodeMap = new Map(); // 保存右侧必要的节点
    ...
    const addColor = (arr, index, textIndex, len, color, clear = true) => {
        ...
        while (len > 0) {
            ...
            let renderLen = Math.min(arr[index].text.length - textIndex, len); // 本次循环渲染的长度
            let text = arr[index].text.substr(textIndex, renderLen);
    ...
    let rightTextNodes = [...rightNodeMap.values()].filter((item => item.type === '#text')); // 左侧的纯文本节点
    let leftTextNodes = [...leftNodeMap.values()].filter((item => item.type === '#text')); // 右侧的纯文本节点
    diff.forEach((part) => {
        if (part.added) {
    ...
    // 找出左右的图片节点进行对比
    let leftImgNodes = [...leftNodeMap.values()].filter((item => item.type === 'IMG'));
    let rightImgNodes = [...rightNodeMap.values()].filter((item => item.type === 'IMG'));
    ...
    // 左右公式节点进行对比，替换
    let leftMathNodes = [...leftNodeMap.values()].filter((item => item.type === 'TEMPLATE' && item.node.id === 'kb-math'));
    let rightMathNodes = [...rightNodeMap.values()].filter((item => item.type === 'TEMPLATE' && item.node.id === 'kb-math'));
